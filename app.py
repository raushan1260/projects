...
def predict_disease_from_symptom(symptom_list):
    ...
    if len(diseases) == 0:
        ...
        disease_details = getDiseaseInfo(most_likely_disease)
        return f"<b>{most_likely_disease}</b><br>{disease_details}"
    else:
        most_likely_diseases = ', '.join(list(diseases))
        return "The most likely diseases are<br><b>" + most_likely_diseases + "</b>"
...
def getDiseaseInfo(disease_name):
    try:
        with DDGS() as ddgs:
            results = ddgs.search(disease_name)
            if results:
    ...
    if request.args["message"] == "undefined":
        ...
        if currentState == 4:
            ...
            if '1' in user_message or 'disease' in user_message:
                disease = predict_disease_from_symptom(all_result['symptoms'])
                response.append("<b>The following disease may be causing your discomfort</b>")
                response.append(disease)
                response.append(
                    f'<a href="https://www.google.com/search?q={disease} disease hospital near me" target="_blank">Search Near By Hospitals</a>')
